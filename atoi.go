...
func myAtoi(s string) int {
    ...
    for i, runeval := range s {
        ...
        if runeval == 0x2d && started == false { //a negative sign is discovered
            ...
            started = true
            newResult = result * 10 + (runeval - 0x30); //push up the digits, add the new value
            if int64(newResult) != int64(result) * 10 + int64(runeval-0x30) { //overflow checker
                if (signedness == 1){
            ...
            result = newResult //shift decimal and add the new value
        }
    }
